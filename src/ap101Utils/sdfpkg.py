...
VENDORED 2026-08-04 from ext/virtualagc yaShuttle/ported/sdfpkg/sdfpkg.py,
which upstream deleted in 85a46336b ("Remove the abandoned first attempt at
an SDFPKG port").  Upstream's replacements -- modules/sdf (class sdf, over a
cmem memory model) and modules/sdfpkg (the MONITOR(22) service dispatcher) --
are not file-based readers and so cannot serve ap101Utils.sdf's query API.
This copy carries our fixes: EBCDIC name fields via set_name_encoding(), and
open() accepting a bare page stream as PASS3 emits it.  Depends on nothing
but the standard library.  Tests: test/test_sdfpkg.py.
...
# Name-field text encoding.  The sdfpkg synthetic flat-file format stores
# symbolic names in ASCII, but a *true* SDF as emitted by HALSFC-PASS3
# retains the original EBCDIC.  set_name_encoding('cp037') switches both
# the decode (returned names) and encode (sought keys, including the
# blank padding, 0x40 in EBCDIC) sides so binary searches compare in the
# stored collation.
NAME_ENCODING = 'ascii'

def set_name_encoding(encoding: str) -> None:
    global NAME_ENCODING
    NAME_ENCODING = encoding

def _field_to_str(data, length: int) -> str:
    """Convert a blank-padded fixed-length name field to a Python str."""
    s = bytes(data[:length])
    return s.decode(NAME_ENCODING, errors='replace').rstrip(' \x00')

def _str_to_field(s: str, length: int) -> bytes:
    """Convert a Python str to a blank-padded fixed-length name field."""
    b = s.encode(NAME_ENCODING, errors='replace')
    pad = ' '.encode(NAME_ENCODING)
    if len(b) >= length:
        return b[:length]
    return b + pad * (length - len(b))
...
class _BlktCell:
    ...
    def __init__(self, data, off: int = 0):
        ...
        self.blk_name   = raw_name.decode(NAME_ENCODING, errors='replace')
...
class SdfContext:
    ...
    def _find_member(self, name8: bytes):
        ...
        if len(hdr) != 8 or hdr[:4] != FLAT_MAGIC:
            # Not a flat container: accept a bare page stream (a single
            # member exactly as emitted by HALSFC-PASS3 into its SDFLIB
            # directory).  Any member name selects the whole file.
            self._fp.seek(0, 2)
            size = self._fp.tell()
            if size and size % PAGE_SIZE == 0:
                return 0, size // PAGE_SIZE
    ...
    def _do_symbol_fill(self, symb_no: int, sn: _SymbNode,
        ...
        full_name = name_bytes.decode(NAME_ENCODING, errors='replace').rstrip()
