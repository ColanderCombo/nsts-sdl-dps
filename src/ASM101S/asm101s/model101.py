...
from .parserAsm import parserASM
from .ibmHex import *
...
def optimizeScratch():
    ...
    for sect in sects:
        ...
        for i in range(len(scratch)):
            ...
            # Note: This checks the USING base address (u[2]) rather than
            # the displacement (value - u[2]).  This works as a heuristic
            # because small base addresses correlate with small data sections
            # whose displacements all fit SRS range. Changing to a proper
            # displacement check would produce functionally-equivalent but
            # byte-different encodings that break compatibility with the
            # original IBM assembler output.
            b = None
            d = 10000000
        ...
        for propNum in range(len(source)):
            ...
            if operation in ["DC", "DS"]:
                ...
                for suboperand in flattened:
                    ...
                    # Z type has a completely different format - handle it before common processing
                    if suboperandType == "Z":
