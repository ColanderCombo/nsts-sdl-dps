...
def writeObjectModule(filename, metadata, symtab, sects, entries, extrns):
    ...
    for r in metadata.get('relocations', []):
        ...
        if r.get('type') == 'Z':
            ...
            # The ZCON data format is [addr_hi, addr_lo, flags, 0] where:
            #   - Bytes 0-1: 16-bit halfword address (filled by linker)
            #   - Byte 2: ZCON flags (MSB=1 means expand with BSR, MSB=0 means expand with 0000)
            #   - Byte 3: Reserved
            # Note: Do NOT use 0x84 - that sets the sign bit and causes negation!
            #
            # QUESTIONABLE: The AP-101S object format spec (AP101_OBJECT_FORMAT_SPEC.md)
            # documents ZCON flags as 0x20/0x40/0x50, not 0x04.  Flag 0x04 works with
            # the current LNK101S because it falls through to the 2-byte length case,
            # but it doesn't match the documented AP-101S flag encoding.  The HAL/S
            # compiler (PASS2) may use different flag values.  If ZCON relocations
            # behave incorrectly, revisit this flag value.
            rldFlags = 0x04
        elif r.get('type') == 'Y':
