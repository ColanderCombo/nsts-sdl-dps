...
function(uv_initialize)
    ...
    # Touch the files/dirs first so REAL_PATH doesn't warn about non-existent paths.
    get_filename_component(_pyproject_dir "${UV_PYPROJECT_FILE}" DIRECTORY)
    if(_pyproject_dir STREQUAL "")
        set(_pyproject_dir "${CMAKE_CURRENT_SOURCE_DIR}")
    endif()
    file(MAKE_DIRECTORY "${_pyproject_dir}")
    file(TOUCH "${UV_PYPROJECT_FILE}")
    ...
    file(MAKE_DIRECTORY "${UV_WORKSPACE_VENV}")
    ...
    execute_process(
        COMMAND
            ${UV} venv ${UV_WORKSPACE_VENV} --python ${UV_PYTHON_VERSION} --allow-existing
        WORKING_DIRECTORY
            ${CMAKE_BINARY_DIR}
    ...
    add_custom_target(uv_sync ALL COMMAND
        ${CMAKE_COMMAND} -E env
            ...
            UV_PROJECT_ENVIRONMENT=${UV_WORKSPACE_VENV}
            # Silence warning if the user's terminal is an a venv
            VIRTUAL_ENV=${UV_WORKSPACE_VENV}
